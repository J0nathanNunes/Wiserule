  ...
  try {
    ...
    if (archivo && archivo instanceof File) {
      ...
      if (bytes.length > config.maxFileSizeMb * 1024 * 1024) {
        return c.json({ status: 'error', error: `Archivo muy grande. M+皤imo: ${config.maxFileSizeMb}MB` }, 413);
      }
    ...
    // --- FASE 5: Generar an+璱isis (en background via Durable Object) ---
    ...
    c.executionCtx.waitUntil(
      (async () => {
        try {
          ...
          await doObj.fetch(`https://tarea/${id}/actualizar`, {
            method: 'POST',
            body: JSON.stringify({ status: 'concluido', progreso: 100, etapa_actual: 'An+璱isis concluida.', relatorio_completo: relatorio }),
          });
        } catch (e) {
          await doObj.fetch(`https://tarea/${id}/actualizar`, {
            method: 'POST',
...
app.post('/api/extrair', async (c) => {
  ...
  try {
    ...
    if (!archivo || !(archivo instanceof File)) {
      return c.json({ status: 'error', error: 'Ning+好 archivo enviado.' }, 400);
    }
...
app.get('/api/historico/:analiseId', async (c) => {
  ...
  if (!analise) return c.json({ status: 'error', error: 'An+璱isis no encontrada.' }, 404);
...
app.get('/api/diagnostico', (c) => {
  ...
  const apis: Record<string, { nombre: string; status: string; detalle: string }> = {
    ...
    minhareceita: { nombre: 'MinhaReceita (CNPJ)', status: 'online', detalle: 'API p+在lica' },
    tavily: {
      nombre: 'Tavily (B+存queda)',
