...
async def health_detalhado():
    ...
    # Apenas verifica configuração (sem chamadas externas para evitar timeout)
    resultados["apis"]["openrouter"] = {"status": "online" if settings.OPENROUTER_API_KEY else "nao_configurada", "configurada": bool(settings.OPENROUTER_API_KEY)}
    resultados["apis"]["minhareceita"] = {"status": "online", "configurada": True}
    resultados["apis"]["supabase"] = {"status": "online" if settings.SUPABASE_URL and settings.SUPABASE_KEY else "nao_configurada", "configurada": bool(settings.SUPABASE_URL and settings.SUPABASE_KEY)}
    resultados["apis"]["tavily"] = {"status": "online" if settings.TAVILY_API_KEY else "nao_configurada", "configurada": bool(settings.TAVILY_API_KEY)}
    resultados["apis"]["geranet"] = {"status": "online" if settings.GERANET_API_KEY else "nao_configurada", "configurada": bool(settings.GERANET_API_KEY)}
